...
async function call(host: string, path: string, init: RequestInit = {}) {
  if (cooldownLeft())
    throw new Error(
      `Rate-limited by LibreLinkUp. Retrying in ${cooldownLeft()}s.`,
    );
  ...
  if (THROTTLED.has(res.status)) {
    ...
    cooldownUntil =
      Date.now() + (retryAfter > 0 ? retryAfter * 1000 : DEFAULT_COOLDOWN);
    throw new Error(
      `Rate-limited by LibreLinkUp. Retrying in ${cooldownLeft()}s.`,
    );
  }
...
async function login(
  ...
  const json = await call(host, "/llu/auth/login", {
    method: "POST",
    headers: headers(),
    body,
  });
  ...
  if (!ticket?.token || !userId) {
    throw new Error(
      `Login failed (status ${json?.status}). Check LIBRE_EMAIL / LIBRE_PASSWORD.`,
    );
  }
  return {
    host,
...
async function getSession() {
  ...
  if (!email || !password)
    throw new Error("Set LIBRE_EMAIL and LIBRE_PASSWORD in .env");

  // authTicket.expires is a unix timestamp in seconds.
  if (session && session.ticket.expires * 1000 > Date.now() + 60_000)
    return session;
...
const pick = (m: any, mmol: boolean) =>
  mmol ? Number(m.Value) : Number(m.ValueInMgPerDl);
...
async function getPatient(s: NonNullable<typeof session>) {
  ...
  if (!conn)
    throw new Error(
      "No connections. Accept a sharing invite in the LibreLinkUp app first.",
    );
  fallbackConn = conn;
...
async function fetchSnapshot(): Promise<Snapshot> {
  ...
  const graph = await call(s.host, `/llu/connections/${id}/graph`, {
    headers: auth(s),
  });
  ...
  if (!m)
    throw new Error("No recent readings. Is the sensor connected and syncing?");
